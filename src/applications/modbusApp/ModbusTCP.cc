...
/*
 ...
 communicate via the Modbus protocol. It is based on the libmodbus library.
 It was adapted to support INET/OMNET++.
...
int ModbusTCP::buildQueryBasis(int slave, int function, int start_addr, int nb, uint8_t *query)
{
...
uint8_t ModbusTCP::computeQueryLengthHeader(int function)
{
	...
	if (function <= FC_FORCE_SINGLE_COIL || function == FC_PRESET_SINGLE_REGISTER)
		/* Read and single write */
		length = 4;
	else if (function == FC_FORCE_MULTIPLE_COILS || function == FC_PRESET_MULTIPLE_REGISTERS)
		/* Multiple write */
		length = 5;
	else
		length = 0;
...
int ModbusTCP::computeQueryLengthData(uint8_t *msg)
{
	...
	if (function == FC_FORCE_MULTIPLE_COILS || function == FC_PRESET_MULTIPLE_REGISTERS)
	...
	else if (ret == COMM_TIME_OUT)
	{
		...
		if (response_length == (offset + 3 + CHECKSUM_LENGTH_TCP))
		{
			...
			if (0x80 + query[offset + 1] == response[offset + 1])
			{
				...
				{
					errorTreat(-exception_code, TAB_ERROR_MSG[response[offset + 2]]);
...
int ModbusTCP::responseIOStatusU8(int address, int nb, tables tab_id, uint8_t *response, int offset)
{
	int shift = 0;
	int byte = 0;
	int i;
...
int ModbusTCP::responseException(sft_t *sft, int exception_code, uint8_t *response)
{
	int response_length;
...
{
	if (error_handling == FLUSH_OR_RECONNECT_ON_ERROR || error_handling == NOP_ON_ERROR)
	{
 ...
 */
bool ModbusTCP::memoryAllocation(int nb_coil_status, int nb_input_status, int nb_holding_registers,
...
void ModbusTCP::initMemory()
{
	...
	if (memoryAllocation(_UT_COIL_STATUS_ADDRESS + UT_COIL_STATUS_NB_POINTS,
			UT_INPUT_STATUS_ADDRESS + UT_INPUT_STATUS_NB_POINTS, UT_HOLDING_REGISTERS_ADDRESS
					+ UT_HOLDING_REGISTERS_NB_POINTS, UT_INPUT_REGISTERS_ADDRESS
					+ UT_INPUT_REGISTERS_NB_POINTS))
	{

		/** INPUT STATUS **/
		setBitsFromBytesU8(UT_INPUT_STATUS_ADDRESS, UT_INPUT_STATUS_NB_POINTS, UT_INPUT_STATUS_TAB,
				INPUT);
...
void ModbusTCP::setBitsFromBytesU8(int address, int nb_bits, const uint8_t tab_byte[],
		tables tab_id)
{
	int i;
	int shift = 0;
...
uint8_t ModbusTCP::getByteFromBits(const uint8_t *src, int address, int nb_bits)
{
	int i;
	uint8_t value = 0;
