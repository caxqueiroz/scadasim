...
struct serverConnection {
    ...
    serverConnection(IPvXAddress a, int port, int profileID, int ratio,
    ...
    serverConnection(IPvXAddress a, int port, int profileID, int ratio,
            int moduleId, ConnectionManager *cm) {
        address = a;
        this->port = port;
        this->profileID = profileID;
        this->ratio = ratio;
...
struct foreignNetwork {
    ...
    foreignNetwork(ConnectionManager *c, int moduleId, int ratio,
...
public:
    ...
    void registerServer(IPvXAddress address, int port, int profileID,
            int moduleId = -1);
    /// @brief Request traffic profile and according server
    void getServer(TargetInfo &ti, TrafficProfile &tp, int &expect,
            int moduleId = -1);
...
public:
    ...
    ConnectionManager *get() {
        ...
        return check_and_cast<ConnectionManager*>(
                simulation.getModuleByPath("connectionManager"));
    }
    ...
    ConnectionManager *get(std::string asName) {
        ...
        return check_and_cast<ConnectionManager*>(
                simulation.getModuleByPath(asName.c_str()));
    }
};
